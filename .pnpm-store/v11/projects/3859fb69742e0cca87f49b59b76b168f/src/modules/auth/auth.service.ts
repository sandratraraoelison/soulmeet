...
import { AccountStatus, AuthProvider, Prisma, Role, SexualOrientation, User } from '@prisma/client';
import {
...
export class AuthService {
  ...
  async login(dto: LoginDto, deviceInfo?: string, ipAddress?: string) {
    ...
    const user = storedUser ? await this.reactivateExpiredSuspension(storedUser) : null;
    if (
      !user?.passwordHash ||
      !user.isActive ||
  ...
  async completeTwoFactorLogin(
    ...
    if (!user?.twoFactorEnabled || !user.twoFactorSecret) {
      await this.auditAdminLogin(user, false, ipAddress, 'Two-factor not enabled');
  ...
  private async finishExternalAuth(
    ...
    user = await this.reactivateExpiredSuspension(user);
  ...
  private async reactivateExpiredSuspension(user: User): Promise<User> {
    if (user.accountStatus !== AccountStatus.SUSPENDED || !user.suspendedUntil || user.suspendedUntil > new Date()) return user;
    return this.prisma.user.update({
      where: { id: user.id },
      data: { isActive: true, accountStatus: AccountStatus.ACTIVE, suspendedUntil: null, moderationReason: null },
    });
  }
