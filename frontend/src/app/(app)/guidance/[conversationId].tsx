...
export default function GuidanceChatScreen() {
  ...
  const menu = (message: GuidanceMessage) => {
    const actions = message.role === 'USER'
      ? [
          { text: 'Delete', style: 'destructive' as const, onPress: () => remove.mutate(message.id) },
        ]
      : [
          { text: 'Regenerate', onPress: () => regenerate.mutate(message.id) },
          { text: 'Delete', style: 'destructive' as const, onPress: () => remove.mutate(message.id) },
        ];
    Alert.alert('Message actions', undefined, [...actions, { text: 'Cancel', style: 'cancel' }]);
  };
  ...
  return (
    <SafeAreaView className="flex-1 bg-canvas">
      ...
      <FlatList inverted data={messages} keyExtractor={(item) => item.id} contentContainerStyle={{ padding: 16 }} renderItem={({ item }) => <GuidanceMessageBubble message={item} onLongPress={() => menu(item)} />} onEndReached={() => history.hasNextPage && !history.isFetchingNextPage && void history.fetchNextPage()} ListHeaderComponent={stream.streamingText ? <View className="mb-3 max-w-[86%] rounded-[20px] rounded-bl-sm border border-primary/30 bg-surface px-4 py-3"><Text className="font-body text-base leading-6 text-ink">{stream.streamingText}</Text></View> : null} ListEmptyComponent={!history.isLoading ? <View className="mt-24 items-center"><Text className="font-headline text-2xl font-bold text-ink">What’s on your mind?</Text><Text className="mt-2 text-center font-body text-muted">Share as much or as little as you want.</Text></View> : null} />
