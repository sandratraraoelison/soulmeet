  ...
  async evaluate(userId: string, mine: SemanticEntry[], candidates: SemanticCandidate[]): Promise<Map<string, SemanticEvaluation> | null> {
    ...
    try {
      const response = await this.llm.complete([
        { role: 'system', content: `You are Soulmeet's multilingual semantic compatibility evaluator. Return JSON only. Normalize every entry into concept, intent, polarity, topics, normalizedMeaning and confidence. Then score each candidate from 0 to 100. Detect paraphrases across languages and explicit or implicit contradictions, especially boundaries and deal breakers. Do not infer protected or highly sensitive traits. Prompt version: ${version}.` },
        { role: 'user', content: `Analyze this JSON and return {"normalizedEntries":[{"id":"...","concept":"...","intent":"...","polarity":"POSITIVE|NEGATIVE|NEUTRAL","topics":["..."],"normalizedMeaning":"...","confidence":0.0}],"matches":[{"candidateId":"...","semanticScore":0,"confidence":0.0,"compatibleConcepts":["..."],"contradictions":["..."],"reasons":["..."]}]}\n${JSON.stringify(payload)}` },
